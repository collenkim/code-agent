...
**API 없이 쓸 수 있다.** 프롬프트를 뽑아 LLM 채팅창에 붙여넣고, 응답을 되돌려 넣는 수동 모드가 기본 사용법이다. 사람이 할 일은 `next` 와 `apply` 두 명령뿐이고, 어디까지 왔는지는 코드가 기억한다. `code-agent serve` 로 [서버와 화면](#서버-모드--화면으로-왕복)을 띄우면 그 왕복이 복사·붙여넣기만 남는다.
...
`turn.ts`가 애초에 전송을 담지 않게 쓰여 있어서, 서버는 그 위에 얹힌 얇은 층이다.
...
없어지는 것은 왕복마다 끼어들던 두 가지다: **응답을 파일로 저장하는 일**과 **명령을 다시 부르는 일**.
사람이 하는 일은 복사와 붙여넣기만 남는다.

```
[복사] 프롬프트  →  Console 에 붙여넣기  →  응답 복사  →  [붙여넣기] [반영하기]
                                                              ↑                 │
                                                              └─────────────────┘
```
...
작업 하나 = **out 디렉토리 하나**다. 진행 상태(계획·세션·질문)는 그 안에 있으므로 서버가 따로 들고 있지 않고,
**서버를 껐다 켜도 이어진다.** 여러 도메인을 동시에 진행할 수 있다.

작업을 만들 때 매니페스트·컨벤션·참조 표준을 실제로 읽어 본다 — 경로 오타가 첫 프롬프트 요청이 아니라
작업을 만드는 자리에서 드러난다.

### HTTP API

화면이 쓰는 것과 같은 API다. 다른 서비스가 그대로 호출해도 된다.
